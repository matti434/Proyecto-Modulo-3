...
export const AdminHomeView = ({
  ...
  return (
    <div className="contenedor-tabla">
      ...
      <section className="seccion-home-admin" style={{ marginTop: "2rem" }}>
        <h3>Equipo (Nosotros)</h3>
        <p className="texto-ayuda-admin">
          Imágenes y datos que se muestran en la página Nosotros. Podés subir o cambiar la foto y eliminar integrantes.
        </p>
        <div className="tabla-responsive">
          <table className="tabla-administracion">
            <thead>
            ...
            <tbody>
              ...
              {equipo?.map((integrante) => (
                <tr key={integrante.id ?? integrante._id}>
                  ...
                  <td>
                    <label className="boton-accion-admin pequeno">
                      {equipoActualizandoId === (integrante.id ?? integrante._id) ? "Subiendo..." : "Subir / Cambiar foto"}
                      <input
                        type="file"
                        accept="image/jpeg,image/jpg,image/png,image/gif,image/webp"
                        onChange={(ev) => {
                    ...
                    {onEliminarIntegranteEquipo && (
                      <button
                        type="button"
                        className="boton-accion-admin pequeno peligro ms-1"
                        onClick={() => {
                          const id = integrante.id ?? integrante._id;
                          if (id) onEliminarIntegranteEquipo(id);
                        }}
                        disabled={!!equipoActualizandoId}
                      >
                        Eliminar
